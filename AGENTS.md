...
- **Dual-Engine Multimodal AI Pipeline (Gemini + xAI Grok)**: Powered by Google Gemini (`@google/genai`) and xAI Grok (`grok-2-latest`).
  - **Stage 1 (Gemini Vision Engine)**: Heavy lifting multimodal visual analysis on intake photos to extract facts (maker marks, serial numbers, visible condition, materials).
  - **Stage 2 (xAI Grok / Listing Engine)**: Uses `XAI_KEY` / `XAI_API_KEY` to generate product identification, 5-part customer descriptions, SEO titles, local FB comps, eBay comps, demand score, and dynamic 5-tier pricing matrix. Falls back seamlessly to Gemini if `XAI_KEY` is not present.
