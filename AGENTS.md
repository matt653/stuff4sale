...
- **Dual-Engine Multimodal AI Pipeline (Gemini + xAI Grok)**: Powered by Google Gemini (`@google/genai`) and xAI Grok (`grok-2-vision-1212`).
  - **Isolated Provider Execution**: `server.ts` and `netlify/functions/api.ts` enforce strict provider isolation. Clicking `⚡ Grok Find It!` executes xAI Grok exclusively without falling through to Gemini. Clicking `✨ Gemini Find It!` executes Google Gemini (`gemini-2.5-flash`, `gemini-2.0-flash`, `gemini-2.0-flash-lite`) exclusively. `🚀 Dual Comparison` runs both in parallel.
  ...
  - **Stage 2 (Google Gemini Listing & Strategy)**: Takes Grok's vision identification and generates the **5-Part Customer Description**, **SEO Title**, **Local FB Comps**, **eBay Comps**, **Demand Score**, **Keywords**, and **5-Tier Strategy Matrix**.
