...
const cmd = `curl.exe -4 -s -k --max-time 5 "https://yvekxqeltflessrblfbq.supabase.co/rest/v1/Stuff4Sale?select=${selectFields}&order=created_at.desc&limit=100" -H "apikey: sb_publishable_qOutgWZwuKaCPODigqZCkw_t7KxhsLm" -H "Authorization: Bearer sb_publishable_qOutgWZwuKaCPODigqZCkw_t7KxhsLm"`;

console.log("Executing super lightweight curl command...");
exec(cmd, { maxBuffer: 50 * 1024 * 1024 }, (err, stdout, stderr) => {
  if (err) {
    console.error("CURL EXEC ERROR:", err);
    return;
  }
  console.log("CURL SUCCESS! STDOUT LENGTH:", stdout.length);
  try {
