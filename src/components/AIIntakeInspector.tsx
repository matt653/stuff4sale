        ...
        <div className="lg:col-span-6">
          {!researchResult && !loading ? (
            ...
            <div className="space-y-4 animate-fade-in" id="evaluation-verdict-card">
              ...
              {/* 5-Tier Pricing & Platform Breakdown Preview */}
              {(() => {
                const tiers = researchResult.pricingTiers || [
                  {
                    tierName: "Low End (Sell Immediately)",
                    percentageLabel: "100%",
                    price: estimatedMin,
                    whereToList: "Facebook Marketplace local pickup, OfferUp, or quick local garage sale",
                    howToList: "List as-is for quick cash pickup within 24 hours."
                  },
                  {
                    tierName: "1/4 Tier (Fast Flip)",
                    percentageLabel: "75%",
                    price: Math.round(estimatedMin + 0.25 * (estimatedMax - estimatedMin)),
                    whereToList: "Facebook Marketplace local pickup, OfferUp",
                    howToList: "Clean basic wipe, 3-4 photos, fast local sale."
                  },
                  {
                    tierName: "Mid End (Fair Market)",
                    percentageLabel: "50%",
                    price: Math.round(estimatedMin + 0.50 * (estimatedMax - estimatedMin)),
                    whereToList: "Cross-list on eBay & Facebook Marketplace",
                    howToList: "Full photos, clear condition details, ship on eBay / pickup on FB."
                  },
                  {
                    tierName: "3/4 Tier (Patient Sale)",
                    percentageLabel: "25%",
                    price: Math.round(estimatedMin + 0.75 * (estimatedMax - estimatedMin)),
                    whereToList: "eBay Buy-It-Now, Mercari",
                    howToList: "Studio background photos, patient ask price hold."
                  },
                  {
                    tierName: "High End (Top Dollar)",
                    percentageLabel: "1%",
                    price: estimatedMax,
                    whereToList: "eBay Buy-It-Now with Best Offer, Specialized Forums",
                    howToList: "Deep restore/clean, test video proof, hold for collector buyer."
                  }
                ];
                  ...
                  <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-2xl space-y-2.5">
                    <div className="flex items-center justify-between text-xs font-extrabold text-slate-800">
                      <span className="flex items-center gap-1.5">
                        ...
                        5-Tier Reselling Strategy Breakdown:
                      </span>
                      <span className="text-[10px] text-slate-500 font-bold">Low End vs Top Dollar</span>
                    </div>

                    <div className="grid grid-cols-5 gap-1 text-center text-[10px]">
                      {tiers.map((t, idx) => (
                        <div key={idx} className="bg-white border border-slate-200 p-1.5 rounded-xl space-y-0.5 shadow-xs">
                          <span className="font-extrabold text-slate-900 block truncate">{t.percentageLabel}</span>
                          <span className="font-black text-indigo-600 block">${t.price}</span>
                        </div>
                      ))}
                    </div>

                    <div className="bg-white border border-slate-200 p-2.5 rounded-xl text-[11px] space-y-1">
                      <div className="flex items-start gap-1 font-semibold text-rose-950">
                        <MapPin size={12} className="text-rose-500 shrink-0 mt-0.5" />
                        <span><b>Fast Cash:</b> {tiers[0].whereToList}</span>
                      </div>
                      <div className="flex items-start gap-1 font-semibold text-indigo-950">
                        <ListChecks size={12} className="text-indigo-600 shrink-0 mt-0.5" />
                        <span><b>Top Dollar Strategy:</b> {tiers[4].howToList}</span>
                      </div>
                    </div>
                  </div>
                );
